...
<div class="ossBody">
<%-- 				<s:iterator value="allOSsBean.oss" var="os"> --%>
				<%
				...
				%>
				<div id="OS<%=os.getOriginalSignatureId() %>">
					<table cellpadding="10"  cellspacing="2" width="95%" border="1" rules="none" style="border-style:dashed;" bordercolor="#428bca" align="center">
						<tr >
						<td rowspan='3' valign='top' style="color:red;"><img src='<%=request.getContextPath()%>/web/img/photo/user<%=os.getOsOwer().getUserId() %>_2.png'><%=os.getOsOwer().getUserName() %></td>	
...
<%-- 					${os.osSubmitDate}	 --%>
						...
						<a href='##' onClick="removeOS(<%=os.getOriginalSignatureId()%>)">删除</a>
						</td>
					</tr>
<!-- 					下面这个tr是评论区，一般是隐藏的 -->
				...
				<table style="margin: 30px auto 30px 50px;" cellpadding="4"
								...
								<tr>
									...
									首页 上一页
										...
										<s:else>
											<a href="showMyOSsAction?allOSsBean.page.currentPage=1">首页</a>
											<a href="showMyOSsAction?allOSsBean.page.currentPage=${allOSsBean.page.currentPage-1}">上一页</a>
										</s:else>
										</td>
									<td>
									<s:if test="%{allOSsBean.page.currentPage!=allOSsBean.page.totalPages}"> 
											<a href="showMyOSsAction?allOSsBean.page.currentPage=${allOSsBean.page.currentPage+1}">下一页</a> 
											<a href="showMyOSsAction?allOSsBean.page.currentPage=${allOSsBean.page.totalPages}">尾页</a> 
 										</s:if> 
 										<s:else> 
										下一页 尾页
 										</s:else> 
